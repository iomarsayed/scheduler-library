...
// SchedulePodGroup schedules the given pods belonging to the same pod group hierarchy using the workload-aware scheduling algorithm.
// If the pods do not belong to the same pod group hierarchy, it returns an error.
func (s *ClusterSnapshot) SchedulePodGroup(ctx context.Context, pods []*v1.Pod, opts SchedulePodGroupOptions) (_ []SchedulingResult, err error) {
	if len(pods) == 0 {
		return nil, nil
	}
