...
func TestSimulatorIntegration_PodGroupScheduling(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			res, err := snap.SchedulePodGroup(ctx, tt.pods, snapshot.NewSchedulePodGroupOptions(false))
			if err != nil {
				t.Fatalf("SchedulePodGroup failed: %v", err)
			}
			if len(res) != tt.wantResultsCount {
				t.Fatalf("SchedulePodGroup returned %d results, want %d", len(res), tt.wantResultsCount)
			}
